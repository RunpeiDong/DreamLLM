...
from omni.models.dreamllm.modeling_plugins import PipelineImageType
from omni.models.dreamllm_sdxl.configuration_dreamllm_sdxl import DreamLLMSDXLConfig as DreamLLMConfig
from omni.models.dreamllm_sdxl.tokenization_dreamllm import (
    DEFAULT_BOS_TOKEN,
...
class DreamLLMSDXLRMSNorm(nn.Module):
...
ALL_LAYERNORM_LAYERS.append(DreamLLMSDXLRMSNorm)
...
class DreamLLMDecoderLayer(nn.Module):
    def __init__(self, config: DreamLLMConfig):
        ...
        self.input_layernorm = DreamLLMSDXLRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = DreamLLMSDXLRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class DreamLLMSDXLModel(DreamLLMPreTrainedModel):
    """DreamLLM-SDXL Model (https://dreamllm.github.io/)
    """
    def __init__(self, config: DreamLLMConfig):
        super().__init__(config)
        if getattr(self.config, "_flash_attn_2_enabled", False):
            logger.info("DreamLLMSDXLModel is using Flash Attention 2")
        ...
        self.norm = DreamLLMSDXLRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class DreamLLMSDXLForCausalMLM(DreamLLMPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config: DreamLLMConfig):
        super().__init__(config)
        self.model = DreamLLMSDXLModel(config)
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)

        self.loss_weight_lm = config.loss_weight_lm
        self.loss_weight_vm = config.loss_weight_vm
        # self.loss_weight_z = config.loss_weight_z
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        model = super(DreamLLMSDXLForCausalMLM, cls).from_pretrained(
        ...
        # HACK: add all pretrained plugins path
    ...
    def forward(
        ...
        output_hidden_states: bool | None = None,
        return_dict: bool | None = None,
    ) -> tuple | CausalLMOutputWithPast: # -> Union[Tuple, CausalLMOutputWithPast]:
        ...
        dream_start_id = self.config.special_tokens2ids_dict["additional_special_tokens"][DEFAULT_DREAM_START_TOKEN]
        dream_end_id = self.config.special_tokens2ids_dict["additional_special_tokens"][DEFAULT_DREAM_END_TOKEN]
        dream_patch_id = self.config.special_tokens2ids_dict["additional_special_tokens"][DEFAULT_DREAM_PATCH_TOKEN]
        ...
        if self.training and images_dm is not None:
            ...
            vm_loss = self.stable_diffusion_head(images_dm, encoder_hidden_states, u_encoder_hidden_states, add_time_ids)

        elif self.training:
            # HACK: dummy forward to avoid `find_unused_parameters` error
            vm_loss = self.stable_diffusion_head(images_dm, None, None, None, self.model.dream_embedding())
