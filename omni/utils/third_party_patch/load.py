...
def _create_importable_file(
    ...
    module_path = ".".join([os.path.basename(dynamic_modules_path), module_namespace, name.replace("/", "--"), hash, name.split("/")[-1]])
    return module_path, hash


def infer_module_for_data_files(data_files_list: DataFilesList, use_auth_token: Optional[Union[bool, str]] = None) -> Optional[Tuple[str, str]]:
    ...
    extensions_counter = Counter(
        suffix.lower() for filepath in data_files_list[: config.DATA_FILES_MAX_NUMBER_FOR_MODULE_INFERENCE] for suffix in Path(filepath).suffixes
    )
...
def infer_module_for_data_files_in_archives(data_files_list: DataFilesList, use_auth_token: Optional[Union[bool, str]]) -> Optional[Tuple[str, str]]:
    ...
    for filepath in data_files_list:
        if str(filepath).endswith(".zip"):
            ...
            archived_files += [
                f.split("::")[0]
                for f in xglob(extracted, recursive=True, use_auth_token=use_auth_token)[: config.ARCHIVED_DATA_FILES_MAX_NUMBER_FOR_MODULE_INFERENCE]
            ]
    ...
    def get_module(self) -> MetricModule:
        ...
        try:
            ...
            if revision is not None:
                ...
                local_path = self.download_loading_script(revision)
                logger.warning(
                    f"Couldn't find a directory or a metric named '{self.name}' in this version. " f"It was picked from the main branch on github instead."
                )
...
class LocalDatasetModuleFactoryWithoutScript(_DatasetModuleFactory):
    ...
    def get_module(self) -> DatasetModule:
        ...
        patterns = sanitize_patterns(self.data_files) if self.data_files is not None else get_data_patterns_locally(base_path)
        data_files = DataFilesDict.from_local_or_remote(
            patterns,
            base_path=base_path,
            allowed_extensions=ALL_ALLOWED_EXTENSIONS,
        )
        split_modules = {split: infer_module_for_data_files(data_files_list) for split, data_files_list in data_files.items()}
    ...
    def get_module(self) -> DatasetModule:
        ...
        if self.data_files is None and self.name in _MODULE_SUPPORTS_METADATA and patterns != DEFAULT_PATTERNS_ALL:
            try:
                ...
                metadata_patterns = None
            if metadata_patterns is not None:
                metadata_files = DataFilesList.from_local_or_remote(metadata_patterns, use_auth_token=self.download_config.use_auth_token, base_path=base_path)
                for key in data_files:
                    data_files[key] = DataFilesList(
                        data_files[key] + metadata_files,
                        data_files[key].origin_metadata + metadata_files.origin_metadata,
...
class HubDatasetModuleFactoryWithoutScript(_DatasetModuleFactory):
    ...
    def get_module(self) -> DatasetModule:
        ...
        patterns = (
            sanitize_patterns(self.data_files) if self.data_files is not None else get_data_patterns_in_dataset_repository(hfh_dataset_info, self.data_dir)
        )
        ...
        if self.data_files is None and module_name in _MODULE_SUPPORTS_METADATA and patterns != DEFAULT_PATTERNS_ALL:
            try:
                ...
                metadata_patterns = None
            if metadata_patterns is not None:
                metadata_files = DataFilesList.from_hf_repo(metadata_patterns, dataset_info=hfh_dataset_info, base_path=self.data_dir)
                for key in data_files:
                    data_files[key] = DataFilesList(
                        data_files[key] + metadata_files,
                        data_files[key].origin_metadata + metadata_files.origin_metadata,
    ...
    **download_kwargs,
    ...
    # Try locally
        ...
        if os.path.isfile(path):
            return LocalDatasetModuleFactoryWithScript(path, download_mode=download_mode, dynamic_modules_path=dynamic_modules_path).get_module()
        else:
            raise FileNotFoundError(f"Couldn't find a dataset script at {relative_to_absolute_path(path)}")
    elif os.path.isfile(combined_path):
        return LocalDatasetModuleFactoryWithScript(combined_path, download_mode=download_mode, dynamic_modules_path=dynamic_modules_path).get_module()
        ...
        try:
            ...
            try:
                ...
                if isinstance(
                    ...
                    raise FileNotFoundError(msg + ". If the repo is private or gated, make sure to log in with `huggingface-cli login`.")
                else:
                    raise e
    ...
    **download_kwargs,
    ...
    with warnings.catch_warnings():
        ...
        if path.endswith(filename):
            if os.path.isfile(path):
                return LocalMetricModuleFactory(path, download_mode=download_mode, dynamic_modules_path=dynamic_modules_path).get_module()
            else:
                raise FileNotFoundError(f"Couldn't find a metric script at {relative_to_absolute_path(path)}")
        elif os.path.isfile(combined_path):
            return LocalMetricModuleFactory(combined_path, download_mode=download_mode, dynamic_modules_path=dynamic_modules_path).get_module()
...
def load_metric(
    ...
    **metric_init_kwargs,
    ...
    with warnings.catch_warnings():
        ...
        download_mode = DownloadMode(download_mode or DownloadMode.REUSE_DATASET_IF_EXISTS)
        metric_module = metric_module_factory(path, revision=revision, download_config=download_config, download_mode=download_mode).module_path
...
def load_from_disk(
    ...
    if fs.isfile(path_join(dest_dataset_path, config.DATASET_INFO_FILENAME)) and fs.isfile(path_join(dest_dataset_path, config.DATASET_STATE_JSON_FILENAME)):
