...
from hf_dataset import list_datasets, load_dataset_by_name
from math_equal import get_answer_str

DATABASE_PATH = "/data/language_eval/database/"
...
DB_NAME = {
    "model_info": "model_info.db",
    "datasets_info": "dataset_info.db",
    # evaluation database, one database per task
    "codegen_eval": "evaluate_info.db",
    "generation_eval": "evaluate_info.db",
    "multi_choice_eval": "evaluate_info.db",
    "math_eval": "evaluate_info.db",
    # dataset db file, all in one
    "human_eval": "dataset.db",
    "math": "dataset.db",
    "evaluate_metrics": "metrics.db",
}
for dataset_name in list_datasets():
    DB_NAME[dataset_name] = "dataset.db"
...
def create_database_table(
    ...
    cmd = f"CREATE TABLE IF NOT EXISTS {table} ({', '.join([f'{h} {t}' for h, t in zip(headers, types)])})"  # noqa
    cursor.execute(cmd)
    cursor.close()
    return conn
...
def add_columns_into_table(conn: sqlite3.Connection, table: str, headers: List[str], types: List[str]):
    ...
    for h, t in zip(headers, types):
        cmd = f"ALTER TABLE {table} ADD COLUMN {h} {t}"
        cursor.execute(cmd)
    cursor.close()
    conn.commit()
...
def select_from_db(cursor: sqlite3.Cursor, table_name, field="all", **kwargs) -> List:
    ...
    cursor.execute(f"SELECT {select_info} FROM {table_name} {where_clause}")
...
def insert_data(
    ...
    cmd = f"INSERT INTO {table} VALUES ({values_str})"
    if many:
...
def update_data(connection: sqlite3.Connection, table: str, update_dict: Dict, close=True, **kwargs):
    ...
    where_clause = where_clause_str(**kwargs)
    cursor.execute(f"UPDATE {table} {update_clause} {where_clause}")
    connection.commit()
    if close:
        connection.close()
    else:
...
def fill_dataset_table(table_name="human_eval"):
    ...
    elif table_name in list_datasets():
...
def delete_table(table_name):
    db_file = HEADERS[table_name], DB_NAME[table_name]
...
def display_data(data, table_name, headers):
    if not data:
        ...
        return data
    for d in data:
        for header, content in zip(headers, d):
...
def get_max_eval_count(model_id, dataset_name="human_eval"):
...
def ensure_model_info_exist(db_file="model_info.db", **kwargs) -> int:
    """ensure model info exists in table and return the id of the model"""

    model_id = get_model_info(field="id", **kwargs)
    if model_id:
        return model_id

    # model info not exist in table, write it into
    copy_kwargs = kwargs.copy()
    if "topp" not in copy_kwargs:
        copy_kwargs["topp"] = -1
    if "topk" not in copy_kwargs:
        copy_kwargs["topk"] = -1
    table_name = "model_info"
    ...
    model_id = select_from_db(conn.cursor(), table_name, "id", **kwargs)
    conn.commit()
    conn.close()
...
def make_parser():
    ...
    parser.add_argument("--task", type=str, default="read", choices=["create_all", "read", "update"])
    parser.add_argument("--table", type=str, default="datasets_info")
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if args.task == "read":
        ...
        # TODO
        pass
        # merge_table("evaluate_info2.db", "evaluate_info.db", "codegen_eval")
