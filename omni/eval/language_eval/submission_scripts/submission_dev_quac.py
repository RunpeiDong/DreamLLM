...
def make_test_prompt(sample, context, title=None):
    question_a = SYSTEM_PROMPT[0] + "Read the passage and judge if the asked question can be answered. Only answer yes or no.\n"
    question = SYSTEM_PROMPT[0] + "Read the passage and give me a brief answer. \n"
        ...
        for para in doc["paragraphs"]:
            ...
            for item in para["qas"]:
                ...
                if is_impossible:
                    ...
                    with torch.no_grad():
                        raw_results = model.generate(input_ids, do_sample=False, temperature=0.0, max_new_tokens=100, stopping_criteria=[stopping_criteria])
                    raw_results = [tokenizer.decode(raw_results[0].tolist(), skip_special_tokens=True)]
                    # if "\n\nAssistant:" in questions[0]:
                ...
                if total % 5 == 0:
                    ...
                    logger.info("Include: {}, Accuracy: {}, F1 score: {}".format(np.mean(incs), np.mean(cors), np.mean(f1s)), end="\r")
                    del _res
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_data_path",
        type=str,
        default="./data/language_official_benchmarks/quac/quac_dev.json",
    )
