...
def evaluate_boolq(args):
    ...
    with open(args.test_data_path, "r", encoding="utf-8") as f:
        for idx, item in tqdm(enumerate(jsonlines.Reader(f))):
            ...
            correct_end = 1 if item["answer"] else 0
            len_ch = len(choices)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_data_path",
        type=str,
        default="./data/language_official_benchmarks/boolq-dev/boolq_dev.jsonl",
    )
    parser.add_argument("--task", "-t", type=str, default="boolq")
