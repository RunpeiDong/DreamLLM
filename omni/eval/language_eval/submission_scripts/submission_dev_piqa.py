...
def make_test_prompt(sample):
    s = f'Question: {sample["goal"]}\nAnswer:'
    return s, [sample["sol1"], sample["sol2"]]


def evaluate_piqa(args):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_data_path",
        type=str,
        default="./data/language_official_benchmarks/physicaliqa-train-dev/dev.jsonl",
    )
    parser.add_argument(
        "--test_label_path",
        type=str,
        default="./data/language_official_benchmarks/physicaliqa-train-dev/dev-labels.lst",
    )
    parser.add_argument("--task", "-t", type=str, default="piqa")
