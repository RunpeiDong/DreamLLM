...
class InfiniteShardList(IterableDataset):
    ...
    def __init__(self, dataset_name, urls, seed=None):
        """Iterate through the list of shards.
    ...
    def __iter__(self):
        ...
        while True:
            for url in urls:
                yield dict(url=url)
            repeat += 1
            logger.warning(f"Repeating {self.dataset_name} for the {repeat}th time.")
...
class ResampledShards(IterableDataset):
    ...
    def __iter__(self):
        ...
        for i in range(self.nshards):
            if i // len(self.urls) > 0:
                logger.warning(f"Repeating {self.dataset_name} for the {i//len(self.urls)}th time.")
