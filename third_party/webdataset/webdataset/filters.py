...
def _interleaved_to_dict(data, *args, handler=reraise_exception, missing_is_error=True, none_is_error=None):
    '''
    args: (png;jpg;json)
    return a dict with keys xxxpng, xxxjpg, xxxjson with corresponding values
    '''
    if none_is_error is None:
        none_is_error = missing_is_error
    if len(args) == 1 and isinstance(args[0], str) and " " in args[0]:
    ...
    for sample in data:
        try:
            ...
            if none_is_error and any(x is None for x in result.values()):
                raise ValueError(f"interleaved_to_dict {args} got {sample.keys()}")
            yield result
        except Exception as exn:
            if handler(exn):
                continue
...
interleaved_to_dict = pipelinefilter(_interleaved_to_dict)
